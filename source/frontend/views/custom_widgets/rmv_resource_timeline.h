...
public:
    ...
    /// Implementation of Qt's sizeHint for this widget.
    /// /return A default sizeHint since the size of this widget can grow to fit
    /// the space allowed by the layout.
    virtual QSize sizeHint() const Q_DECL_OVERRIDE;

    /// Initialize the widget with non-default values.
    /// \param model The model where the timeline data is stored.
    void Initialize(rmv::ResourceDetailsModel* model);

signals:
    /// Indicate that the timeline was clicked on.
    /// \param logical_position The logical position on the timeline clicked on.
    ...
    /// \param tolerance A factor around the logical_position still considered to
