...
/// \file
/// \brief  Implementation of RMV's carousel allocation sizes widget
//=============================================================================
...
void RMVCarouselAllocationSizes::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget* widget)
{
    ...
    int gap       = 5;
    int bar_width = config_.width / (kNumAllocationSizeBuckets + 3);
    int x_offset  = (config_.width - (((kNumAllocationSizeBuckets - 1) * gap) + (kNumAllocationSizeBuckets * bar_width))) / 2;

    for (int index = 0; index < kNumAllocationSizeBuckets; index++)
    {
...
void RMVCarouselAllocationSizes::SetData(const RMVCarouselData& data)
{
