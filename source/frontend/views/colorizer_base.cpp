...
QColor ColorizerBase::GetHeapColor(RmtHeapType heap_type)
{
    ...
    case kRmtHeapTypeLocal:
        return RMVSettings::Get().GetColorHeapLocal();

    case kRmtHeapTypeInvisible:
        return RMVSettings::Get().GetColorHeapInvisible();

    case kRmtHeapTypeSystem:
        return RMVSettings::Get().GetColorHeapSystem();

    case kRmtHeapTypeNone:
        return RMVSettings::Get().GetColorHeapUnspecified();

    default:
        return RMVSettings::Get().GetColorResourceFreeSpace();
    }
}
...
QColor ColorizerBase::GetResourceUsageColor(RmtResourceUsageType usage_type)
{
    ...
    case kRmtResourceUsageTypeDepthStencil:
        out = RMVSettings::Get().GetColorResourceDepthStencil();
        break;
    case kRmtResourceUsageTypeRenderTarget:
        out = RMVSettings::Get().GetColorResourceRenderTarget();
        break;
    case kRmtResourceUsageTypeTexture:
        out = RMVSettings::Get().GetColorResourceTexture();
        break;
    case kRmtResourceUsageTypeVertexBuffer:
        out = RMVSettings::Get().GetColorResourceVertexBuffer();
        break;
    case kRmtResourceUsageTypeIndexBuffer:
        out = RMVSettings::Get().GetColorResourceIndexBuffer();
        break;
    case kRmtResourceUsageTypeUav:
        out = RMVSettings::Get().GetColorResourceUAV();
        break;
    case kRmtResourceUsageTypeShaderPipeline:
        out = RMVSettings::Get().GetColorResourceShaderPipeline();
        break;
    case kRmtResourceUsageTypeCommandBuffer:
        out = RMVSettings::Get().GetColorResourceCommandBuffer();
        break;
    case kRmtResourceUsageTypeHeap:
        out = RMVSettings::Get().GetColorResourceHeap();
        break;
    case kRmtResourceUsageTypeDescriptors:
        out = RMVSettings::Get().GetColorResourceDescriptors();
        break;
    case kRmtResourceUsageTypeBuffer:
        out = RMVSettings::Get().GetColorResourceBuffer();
        break;
    case kRmtResourceUsageTypeGpuEvent:
        out = RMVSettings::Get().GetColorResourceGPUEvent();
        break;
    case kRmtResourceUsageTypeFree:
        out = RMVSettings::Get().GetColorResourceFreeSpace();
        break;
    case kRmtResourceUsageTypeInternal:
        out = RMVSettings::Get().GetColorResourceInternal();
        break;
...
QColor ColorizerBase::GetColor(const RmtVirtualAllocation* const allocation, const RmtResource* const resource) const
{
    ...
    case kColorModeResourceCPUMapped:
        if (resource != nullptr && resource->bound_allocation != nullptr)
        {
            if ((resource->bound_allocation->flags & kRmtAllocationDetailIsCpuMapped) == kRmtAllocationDetailIsCpuMapped)
                return RMVSettings::Get().GetColorCPUMapped();
            else
                return RMVSettings::Get().GetColorNotCPUMapped();
        }
        break;

    case kColorModeNotAllPreferred:
    {
        const RmtDataSnapshot* open_snapshot = TraceManager::Get().GetOpenSnapshot();
        ...
        if (memory_segment_histogram[preferred_heap] != resource->size_in_bytes)
            return RMVSettings::Get().GetColorNotInPreferredHeap();
        else
            return RMVSettings::Get().GetColorInPreferredHeap();
    }
    break;
    ...
    case kColorModeAliasing:
    {
        ...
        {
            return RMVSettings::Get().GetColorAliased();
        }
        else
        {
            return RMVSettings::Get().GetColorNotAliased();
        }
    }
    break;
    ...
    case kColorModeCommitType:
        if (resource != nullptr && resource->bound_allocation != nullptr)
        {
            ...
            case kRmtCommitTypeCommitted:
                return RMVSettings::Get().GetColorCommitTypeCommitted();
            case kRmtCommitTypePlaced:
                return RMVSettings::Get().GetColorCommitTypePlaced();
            case kRmtCommitTypeVirtual:
                return RMVSettings::Get().GetColorCommitTypeVirtual();
            default:
                break;
            }
        }
...
{
    RMT_ASSERT(legends_scene != nullptr);
    ...
    case kColorModePreferredHeap:
    case kColorModeActualHeap:
        legends_scene->AddColorLegendItem(RMVSettings::Get().GetColorHeapLocal(), RmtGetHeapTypeNameFromHeapType(kRmtHeapTypeLocal));
        legends_scene->AddColorLegendItem(RMVSettings::Get().GetColorHeapInvisible(), RmtGetHeapTypeNameFromHeapType(kRmtHeapTypeInvisible));
        legends_scene->AddColorLegendItem(RMVSettings::Get().GetColorHeapSystem(), RmtGetHeapTypeNameFromHeapType(kRmtHeapTypeSystem));
        legends_scene->AddColorLegendItem(RMVSettings::Get().GetColorHeapUnspecified(), RmtGetHeapTypeNameFromHeapType(kRmtHeapTypeNone));
        break;
    ...
    case kColorModeResourceUsageType:
    {
        for (int32_t index = 0; index < kRmtResourceUsageTypeCount; ++index)
        {
            if (index != kRmtResourceUsageTypeUnknown)
            {
                ...
                legends_scene->AddColorLegendItem(GetResourceUsageColor(resource_usage_type), rmv::string_util::GetResourceUsageString(resource_usage_type));
            }
        }
    }
    break;
    ...
    case kColorModeResourceCPUMapped:
        legends_scene->AddColorLegendItem(RMVSettings::Get().GetColorCPUMapped(), "CPU mapped");
        legends_scene->AddColorLegendItem(RMVSettings::Get().GetColorNotCPUMapped(), "Not CPU mapped");
        break;

    case kColorModeNotAllPreferred:
        legends_scene->AddColorLegendItem(RMVSettings::Get().GetColorInPreferredHeap(), "All in preferred heap");
        legends_scene->AddColorLegendItem(RMVSettings::Get().GetColorNotInPreferredHeap(), "Not all in preferred heap");
        break;

    case kColorModeAliasing:
        legends_scene->AddColorLegendItem(RMVSettings::Get().GetColorAliased(), "Aliased");
        legends_scene->AddColorLegendItem(RMVSettings::Get().GetColorNotAliased(), "Not aliased");
        break;

    case kColorModeCommitType:
        legends_scene->AddColorLegendItem(RMVSettings::Get().GetColorCommitTypeCommitted(), "Committed");
        legends_scene->AddColorLegendItem(RMVSettings::Get().GetColorCommitTypePlaced(), "Placed");
        legends_scene->AddColorLegendItem(RMVSettings::Get().GetColorCommitTypeVirtual(), "Virtual");
        break;
...
{
    const RmtDataSnapshot* snapshot = TraceManager::Get().GetOpenSnapshot();
    if (snapshot == nullptr)
    {
        return -1;
    }
