...
namespace rmv
{
    /// Class to generate and store the aliased resource data. This is generated when
    /// a snapshot is created. It consists of a map lookup of allocation to aliased data.
    /// If an allocation isn't in the map, then it isn't aliased.
    ...
    public:
        ...
        /// Clear out the aliased data model.
        void Clear();

        /// Generate the aliased data.
        ...
        /// \param allocation The allocation containing the resources.
        ...
        /// Get the number of rows required for drawing the resources for the current allocation.
        ...
        /// Get the row that a resource is on.
        /// \param allocation The virtual allocation containing the resources.
        /// \param index The index of the resource in the rmt_resource_list structure.
        /// \return The row.
        int GetRowForResourceAtIndex(const RmtVirtualAllocation* allocation, int index) const;

    private:
        /// Struct containing information for aliased resources. Consists of a vector indicating
        /// which row a resource is to be drawn in (the index in the vector is the same as the index
        /// in the resource list in the back end) and the total number of rows needed to show the
        /// resources.
        struct AliasData
        {
