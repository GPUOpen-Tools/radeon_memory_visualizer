...
{
    int32_t a1 = reinterpret_cast<const ResourceMapping*>(r1)->usage_amount;
    int32_t a2 = reinterpret_cast<const ResourceMapping*>(r2)->usage_amount;
    ...
    if (TraceManager::Get().DataSetValid())
    {
        if (base_snapshot != nullptr && diff_snapshot != nullptr)
        {
            ...
            if (base_success && diff_success)
            {
                ...
                // sort the resources by amount
                qsort(
                    (void*)out_carousel_delta_data.resource_types_data.usage_map, kRmtResourceUsageTypeCount, sizeof(ResourceMapping), ResourceSortComparator);
...
{
    RmtDataSnapshot* open_snapshot = TraceManager::Get().GetOpenSnapshot();
